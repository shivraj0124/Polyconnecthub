...
function CollegeInfo() {
  ...
  const getCollege = async () => {
    ...
    try {
      const result = await axios.post(
        `${VITE_BACKEND_API}/api/poc/getOneCollege`,
        {
  ...
  const handleEditCollege = async (e) => {
    ...
    try {
      const result = await axios.post(
        `${VITE_BACKEND_API}/api/poc/editCollegeInfo`,
        {
          id: college._id,
          name: collegeName,
          about: about,
    ...
    <div className="flex flex-col w-full  h-[90vh] p-5 ">
      ...
      <div className="overflow-y-auto flex flex-col mt-5 shadow-md border  p-4 rounded-md bg-white mx-auto md:w-[65%] dark:bg-slate-800 dark:text-white">
        ...
        <div className="flex flex-col  ">
          ...
          <h1 className="mt-5"><span className=" text-green-600 font-semibold">About: </span>{college?.about}</h1>
          <h1 className="mt-3"><span className=" text-green-600 font-semibold">Address: </span>{college?.address}</h1>
        </div>
      </div>
      {isModelOpen && (
        <>
